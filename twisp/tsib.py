...
Record = namedtuple('Record',
                    ['trans_date', 'post_date', 'details', 'amount', 'translation_date', 'country', 'currency',
...
def liberate_data_table(text):
    rows = []
    text = re.sub(r'^(\d{3}/\d{2}/\d{2} \d{3}/\d{2}/\d{2})', r'\n\1', text, flags=re.MULTILINE)
    ...
    for m in matches:
        ...
        row = Record(trans_date, post_date, details, m[3],
                     translation_date, country, currency, m[7])
        rows.append(row)
    return rows
